...
static bool ggml_backend_cpu_device_supports_op(ggml_backend_dev_t dev, const struct ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_SOFT_MAX_BACK:
            // No max_bias gate: the ALiBi bias is additive and constant w.r.t. the logits, so the
            // backward kernel is correct for any bias and does not even read it. See the comment
            // in ggml_compute_forward_soft_max_ext_back_f32.
